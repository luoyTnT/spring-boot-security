package com.example.security.common.exception.handler;
...
public class GlobalExceptionHandler {
    ...
    public ApiResponse<?> handlerException(Exception e) {
        if (e instanceof NoHandlerFoundException) {
            ...
            return ApiResponse.ofStatus(SystemRespCode.REQUEST_NOT_FOUND);
            ...
            return ApiResponse.ofStatus(SystemRespCode.HTTP_BAD_METHOD);
        } else if (e instanceof MethodArgumentNotValidException) {
            log.error("【全局异常拦截】MethodArgumentNotValidException", e);
            return ApiResponse.of(SystemRespCode.BAD_REQUEST.getCode(), ((MethodArgumentNotValidException) e).getBindingResult().getAllErrors().get(0).getDefaultMessage(), null);
        } else if (e instanceof ConstraintViolationException) {
            log.error("【全局异常拦截】ConstraintViolationException", e);
            return ApiResponse.of(SystemRespCode.BAD_REQUEST.getCode(), CollUtil.getFirst(((ConstraintViolationException) e).getConstraintViolations()).getMessage(), null);
            ...
            return ApiResponse.ofStatus(SystemRespCode.PARAM_NOT_MATCH);
        } else if (e instanceof HttpMessageNotReadableException) {
            log.error("【全局异常拦截】HttpMessageNotReadableException: 错误信息 {}", ((HttpMessageNotReadableException) e).getMessage());
            return ApiResponse.ofStatus(SystemRespCode.PARAM_NOT_NULL);
        } else if (e instanceof BadCredentialsException) {
            log.error("【全局异常拦截】BadCredentialsException: 错误信息 {}", e.getMessage());
            return ApiResponse.ofStatus(SystemRespCode.USERNAME_PASSWORD_ERROR);
        } else if (e instanceof DisabledException) {
            log.error("【全局异常拦截】BadCredentialsException: 错误信息 {}", e.getMessage());
            return ApiResponse.ofStatus(SystemRespCode.USER_DISABLED);
        ...
        return ApiResponse.ofStatus(SystemRespCode.ERROR);
    }
}
